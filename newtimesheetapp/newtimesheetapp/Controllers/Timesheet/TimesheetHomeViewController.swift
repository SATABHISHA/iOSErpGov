...
//                           arrColorCode.append(json["DayStatus"])
                          /*  for innerItem in json["DayStatus"].arrayValue {
                            ...
                            }*/ //-------above block is commented for this page but may be required for future use
                           
                            dict = json["DayStatus"] //---newly done this one
                            for inneritem in dict.arrayValue{
                                if inneritem["Description"].stringValue == "Not Started"{
                                    print("Not started dates==>",inneritem["Period"].stringValue)
                                }
                            }
                            print("Dictionary Data2===>",dict!)
...
extension TimesheetHomeViewController: FSCalendarDataSource, FSCalendarDelegate, FSCalendarDelegateAppearance {
    func calendar(_ calendar: FSCalendar, didSelect date: Date, at monthPosition: FSCalendarMonthPosition) {
        ...
        for inneritem in dict.arrayValue{
            if inneritem["Period"].stringValue.contains(dateString){
                 print("Selecetd Date====>",dateString)
                 UserSingletonModel.sharedInstance.selectedDate = dateString
                self.performSegue(withIdentifier: "timesheetSelectDaySegue", sender: self)
            }
        }
    ...
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, fillDefaultColorFor date: Date) -> UIColor? {
        ...
        if UserSingletonModel.sharedInstance.UserType == "MAIN" {
        if dict != nil{
            for inneritem in dict.arrayValue{
                if inneritem["Period"].stringValue.contains(dateString){
//                    return UIColor(hexFromString: inneritem["ColorCode"].stringValue)
                    returnColor = UIColor(hexFromString: inneritem["ColorCode"].stringValue)
                    break
                }
            }
        }else{
...
//            return .clear
      ...
      /*  if self.arrPeriod.contains(dateString){
       ...
       return .clear
        ...
        return .clear*/ //---commented on 3rd july
